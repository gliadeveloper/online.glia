...
export function QuizPlayer({
  ...
  return (
    <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
      ...
      <p className="mt-2 text-xs text-zinc-500">합격 점수 {passingScore}% · {questions.length}문항</p>

      {result ? (
        <div
          className={`mt-6 rounded-xl px-5 py-4 ${
            result.isPassed ? "bg-emerald-50 text-emerald-900" : "bg-amber-50 text-amber-900"
          }`}
        >
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-5">
          {questions.map((question, index) => (
            <fieldset key={question.id} className="rounded-xl border border-zinc-100 bg-zinc-50 p-4">
